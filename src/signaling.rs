...
fn get_led_state(path: &str) -> u8 {
    let mut file = std::fs::File::open(path).expect(format!("Failed to open led device {} for reading", path));
    let mut contents = String::new();
    file.read_to_string(&mut contents).expect(format!("Failed to read current state of led device {}", path));
...
fn set_led_state(path: &str, new_state: u8) {
    let mut out_file = std::fs::File::create(path).expect(format!("Failed to open led device {} for writing", path));
    out_file.write(new_state.to_string()).expect(format!("Failed to write new state for led device {}", path));
}
